...
public class XbimIfcService : IXbimIfcService
{
    ...
    public async Task<IfcMetadata> ParseIfcFileAsync(string ifcFilePath)
    {
        ...
        {
            using var model = IfcStore.Open(ifcFilePath, XbimDBAccess.Read);
            ...
            _logger.LogInformation($"[XBIM] Successfully parsed IFC file. Project: {metadata.ProjectName}, Schema: {metadata.IfcSchema}");
